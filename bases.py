...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2.0, reduction='none', alpha=None):
        """
        ...
        alpha: 
            ...
            - list or tensor (per-class),
            - or 'auto' (compute from class frequencies)
        """
        ...
        if alpha is None:
            self.alpha = None
            self.use_class_alpha = False
        elif alpha == -1:
            print("AUTOMATIC ALPHA")
            self.alpha = 'auto'
            self.use_class_alpha = True
        elif isinstance(alpha, (float, int, list, torch.Tensor)):
            self.alpha = torch.tensor(alpha, dtype=torch.float32)
            print("ALPHA:", self.alpha)
            self.use_class_alpha = True
        else:
            raise ValueError("Unsupported alpha type")

    def compute_auto_alpha(self, targets):
        # targets shape: (batch_size, num_classes)
        class_freq = targets.sum(dim=0)
        total = class_freq.sum()
        alpha = 1.0 - (class_freq / total)
        return alpha
    ...
    def forward(self, inputs, targets):
        ...
        if self.use_class_alpha:
            if isinstance(self.alpha, str) and self.alpha == 'auto':
                alpha_vals = self.compute_auto_alpha(targets).to(inputs.device)
            else:
                alpha_vals = self.alpha.to(inputs.device)

            alpha_weight = torch.where(targets == 1, alpha_vals, 1 - alpha_vals)
